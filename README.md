<p align="center">
  <img src="./img.png" alt="Project Banner" width="100%">
</p>

# [ 🎯campusIQ]

## Basic Details

### Team Name: [nishana]

### Team Members
- Member 1: [DIYANA] - [EMEA arts and science College]
- Member 2: [NISHANA] - [EMEA arts and science College]

### Hosted Project LinK
[https://diyanajaferdiyanajafer.github.io/tink-her-hack-temp/]

### Project Description
[CampusIQ is a web application designed for college management. It enables students to submit complaints, access exam and event information, and interact with a chatbot, while admins can efficiently manage complaints, exams, and events.]


### The Problem statement
[Students and administrators face communication gaps, leading to delays in complaint handling, exam updates, and event notifications.]

### The Solution
[CampusIQ is a web-based platform that allows students to submit complaints, view exams and events, and get quick info via a chatbot, while admins can manage complaints and update schedules in real time.]



## Features

List the key features of your project:
- Feature 1: [Student and Admin login with authentication]
- Feature 2: [Complaint submission and tracking]
- Feature 3: [Dynamic display of exams and events]
- Feature 4: [Hardcoded chatbot for instant assistance]

---

## Implementation
[The CampusIQ project is implemented as a web application using HTML, CSS, and JavaScript for the frontend. Firebase is used for backend services, including Authentication (for student and admin login) and Firestore Database (to store complaints, exam info, and college events). The workflow includes:

User Authentication: Students and admins register/login using Firebase Authentication.

Student Dashboard: Students can submit complaints, view college events, and see the latest exam info. A simple chatbot answers predefined queries.

Admin Dashboard: Admins can view student complaints, update exam details, and add college events. All data is synced in real-time with Firebase Firestore.

Data Management: Firestore collections handle complaints, exams, and events, allowing seamless read/write operations through JavaScript modules.

This setup ensures a responsive, interactive, and real-time campus management system.]



#### Screenshots (Add at least 3)

<img width="1366" height="768" alt="Screenshot 2026-02-14 102257" src="https://github.com/user-attachments/assets/b5789dac-5b95-4965-abfc-dd4e1439dcaa" />
Home Page

<img width="1366" height="720" alt="Screenshot 2026-02-14 102333" src="https://github.com/user-attachments/assets/9916586f-9219-4e0b-8d0e-58e5b8a44bb0" />
Login Page

<img width="1366" height="720" alt="Screenshot 2026-02-14 102423" src="https://github.com/user-attachments/assets/4eaa25bc-c174-47e1-8706-b0160a3647a0" />
student Dashboard

<img width="1366" height="720" alt="Screenshot 2026-02-14 102452" src="https://github.com/user-attachments/assets/0573ea7b-9daa-4a38-bacb-76297e12b984" />
Admin Dashboard










## AI Tools Used (Optional - For Transparency Bonus)

If you used AI tools during development, document them here for transparency:

**Tool Used:** [e.g., GitHub Copilot, v0.dev, Cursor, ChatGPT, Claude]

**Purpose:** [What you used it for]
- Example: "Generated boilerplate React components"
- Example: "Debugging assistance for async functions"
- Example: "Code review and optimization suggestions"

**Key Prompts Used:**
- "Create a REST API endpoint for user authentication"
- "Debug this async function that's causing race conditions"
- "Optimize this database query for better performance"

**Percentage of AI-generated code:** [Approximately X%]

**Human Contributions:**
- Architecture design and planning
- Custom business logic implementation
- Integration and testing
- UI/UX design decisions

*Note: Proper documentation of AI usage demonstrates transparency and earns bonus points in evaluation!*

---

## Team Contributions

- [diyana]: [Frontend Development & UI/UX Design

Designed the student and admin dashboards

Created responsive forms for login, signup, and complaint submission

Implemented chatbot interface and layout.]
- [nishana]: [Backend & Firebase Integration

Integrated Firebase Authentication for student and admin login

Connected Firestore database for complaints, exams, and events

Handled CRUD operations for events, complaints, and exam info.]
- 
---



Made with ❤️ at TinkerHub
